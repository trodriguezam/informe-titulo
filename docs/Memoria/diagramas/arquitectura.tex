...
\begin{tikzpicture}[
  ...
  box/.style={draw=localDraw, line width=0.8pt, rounded corners=3pt, fill=localFill,
              align=center, text width=3.5cm, minimum height=1.0cm, inner sep=4pt},
...
\node[box]                          (ct)     at (0,0)        {Escáner CT de entrada\\(NIfTI)};
\node[box, below=0.7cm of ct]       (prep)                    {Preprocesamiento\\\scriptsize(recorte de unidades Hounsfield)};
\node[box, below=2.0cm of prep]     (consol)                  {Consolidación de\\estructuras};
...
\node[extbox, below=0.9cm of stl]   (prusa)                   {PrusaSlicer\\$\rightarrow$ Prusa XL};

% ---- Servidor remoto (a la derecha) ----
\node[remotebox] (ts) at (7.0,-2.55) {TotalSegmentator\\\scriptsize(tareas de segmentación en GPU)\\[2pt]máscaras por estructura};
...
\draw[flow] (corte)  -- (stl);
\draw[flow] (stl)    -- (prusa) node[lbl, midway] {STL};

% ---- Flechas SCP (ida y vuelta al servidor) ----
\draw[scp] (prep.east) -- (ts.west) node[lbl, pos=0.52] {CT preprocesado (SCP)};
\draw[scp] (ts.west) -- (consol.east) node[lbl, pos=0.48] {máscaras (SCP)};
...
% Módulo de Segmentación: ct .. slicer
...
\node[text=localDraw, font=\scriptsize\bfseries, text width=2.2cm, align=right, anchor=east]
  at ($(ct.west)!0.5!(slicer.west)+(-0.7,0)$) {MÓDULO DE\\SEGMENTACIÓN};

% Módulo de Corte: corte .. stl
\draw[decorate, decoration={brace, amplitude=7pt}, line width=0.8pt, draw=localDraw]
...
% ---- Etiqueta "externo" ----
\node[font=\scriptsize\itshape, text=extDraw, anchor=west] at ([xshift=6pt]prusa.east) {externo al sistema};
